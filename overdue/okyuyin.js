...
function login() {
    return new Promise(async (resolve) => {
        let options = taskPost("app/user/login/login",`equipment=109E7C53-6E1F-46F7-B01B-DF119CE76C5C&loginType=&openId=&password=${okPwd}&phone=${okPhone}&sdkAppId=&source=${okSource}&ticket=&type=1`)
        $.post(options, async (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${JSON.stringify(err)}`);
