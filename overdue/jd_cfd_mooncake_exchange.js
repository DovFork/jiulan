/*
京喜财富岛合成月饼兑换
cron 59 0-23/1 * * * jd_cfd_mooncake_exchange.js    整点有毛自己改
更新时间：2021-11-30
活动入口：微信京喜-我的-京喜财富岛
变量  可选值  20 100 500 1000 10000 （对应金额除100）
export JD_CFD_MOONACKE_DDW_VIRHB="100"
已支持IOS双京东账号,Node.js支持N个京东账号
脚本兼容: QuantumultX, Surge, Loon, JSBox, Node.js
============Quantumultx===============
[task_local]
#京喜财富岛合成月饼兑换
59 0-23/1 * * * https://raw.githubusercontent.com/jiulan/platypus/main/scripts/jd_cfd_mooncake_exchange.js, tag=京喜财富岛合成月饼兑换, img-url=https://raw.githubusercontent.com/58xinian/icon/master/jxcfd.png, enabled=true

================Loon==============
[Script]
cron "59 0-23/1 * * *" script-path=https://raw.githubusercontent.com/jiulan/platypus/main/scripts/jd_cfd_mooncake_exchange.js,tag=京喜财富岛合成月饼兑换

===============Surge=================
京喜财富岛合成月饼兑换 = type=cron,cronexp="59 0-23/1 * * *",wake-system=1,timeout=3600,script-path=https://raw.githubusercontent.com/jiulan/platypus/main/scripts/jd_cfd_mooncake_exchange.js

============小火箭=========
京喜财富岛合成月饼兑换 = type=cron,script-path=https://raw.githubusercontent.com/jiulan/platypus/main/scripts/jd_cfd_mooncake_exchange.js, cronexpr="59 0-23/1 * * *", timeout=3600, enable=true
 */
const $ = new Env("京喜财富岛合成月饼兑换");
...
if ($.isNode()) {
    ...
    if (process.env.JD_CFD_MOONACKE_DDW_VIRHB ) {
        ddwVirHb = process.env.JD_CFD_MOONACKE_DDW_VIRHB
    }
...
!(async () => {
    ...
    await requestAlgo();
    await $.wait(500)
    console.log(`\n\n\n变量JD_CFD_MOONACKE_DDW_VIRHB  可选值  20 100 500 1000 10000 (对应值除100)\n`)

    await getJDtime()
    let timestamp=new Date().getTime();
    let timedifference=timestamp-Number(JDtime);
    console.log(`京东服务器时间戳：`+JDtime);
    console.log(`当前服务器时间戳：`+timestamp);
    console.log(`服务器延迟为`+timedifference+`毫秒`);

    let setdatetemp = (new Date(new Date().setHours(new Date().getHours()+1))).Format("yyyy-MM-dd hh:mm:ss");
    let setdate = setdatetemp.split(":")[0]+":00:00";
    let settimestamp = (new Date(setdate)).getTime();
    ...
    for (let i = 0; i < cookiesArr.length; i++) {
        if (cookiesArr[i]) {
            ...
            if(!ddwVirHb){
                console.log(`未设置  默认提现最大兑换额度\n`)
                await exchangePinPinPearlStateByMax();
            }else {
                let condition = conditionAllList.filter(e => e.ddwVirHb === Number(ddwVirHb))[0];
                if (condition){
                    await exchangePinPinPearl(condition.ddwVirHb,condition.dwLvl,condition.strPool,true);
                }else {
            ...
            if (randHb){
                console.log(`尝试随机金额 \n`)
                await exchangePinPinPearl(randHb.ddwVirHb,randHb.dwLvl,randHb.strPool,false);
            }
            // await $.wait(500);
        }
    }
...
async function exchangePinPinPearlStateByMax(){
    ...
    await $.wait(500);
    await exchangePinPinPearl(condition.ddwVirHb,condition.dwLvl,condition.strPool,false);
}
            ...
            try {
                if (err) {
                    ...
                    if (data) {
                        ...
                        if (data.iRet === 0) {
                            console.log(`京东账号${$.index} ${$.UserName} 兑换成功  金额:【`+ddwVirHb+'】\n');
...
async function exchangePearlState() {
    return new Promise(async (resolve) => {
        $.get(taskUrl(`user/ExchangePearlState`, `__t=${Date.now()}`), async (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${JSON.stringify(err)}`)
                    console.log(`${$.name} ComposePearlState API请求失败，请检查网路重试`)
                    ...
                    if (data) {
                        ...
                        if (data.iRet === 0) {
                            ...
                            if (data.exchangeInfo.randHbPrizeInfo.dwState === 0){
                                randHb = {
                                    "ddwVirHb": undefined,
                                    "dwLvl": undefined,
                                    "strPool":data.exchangeInfo.randHbPrizeInfo.strPool
                                }
                            }else {
                                randHb = undefined;
                            }
...
function getUrlData(url, name) {
    if (typeof URL !== "undefined") {
        ...
        for (let i = 0; i < lets.length; i++) {
            ...
            if (pair[0] === name) {
                // return pair[1];
                return lets[i].substr(lets[i].indexOf('=') + 1);
            }
        }
        return ''
    }
