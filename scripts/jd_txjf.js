...
function sign() {
    return new Promise(async resolve => {
        const options = {
            url: `https://dwapp.jd.com/user/sign?t=714`,
            headers: {
            ...
            try {
                if (err) {
                    ...
                    if (safeGet(data)) {
                        ...
                        if (data.code === 200) {
                            console.log(`签到成功\n`);
                            allMessage += `京东账号${$.index} ${$.UserName}通讯积分：${data.data.balanceNum}\n`;
                        } else {
...
function taskList() {
    return new Promise(async resolve => {
        const options = {
            url: `https://dwapp.jd.com/user/task/list?t=9490`,
            headers: {
                'Origin': 'https://prodev.m.jd.com',
                'Cookie': $.cookie,
                'Connection': `keep-alive`,
                        ...
                        if (data.code === 200) {
                            if (data.data) {
                                for (let v = 0; v < data.data.length; v++) {
                                    if (data.data[v].viewStatus === 0) {
                                        console.log(`执行任务： ` + data.data[v].name)
                                        await record(data.data[v].id,data.data[v].taskType);
                                        await $.wait(500)
                                        await receive(data.data[v].id);
                            ...
                            console.log(`积分任务列表获取异常:${JSON.stringify(data)}\n`);
                        }
                    }
                }
            } catch (e) {
                ...
                if (err) {
                    ...
                    if (safeGet(data)) {
                        ...
                        if (data.code === 200) {
                            ...
                            console.log(`任务记录异常:${JSON.stringify(data)}\n`);
                        }
                    }
                }
            } catch (e) {
...
function receive(id) {
    return new Promise(async resolve => {
        const options = {
            url: `https://dwapp.jd.com/user/task/receive?id=${id}&t=7703`,
            headers: {
                'Origin': 'https://prodev.m.jd.com',
                'Cookie': $.cookie,
                'Connection': `keep-alive`,
            ...
            try {
                if (err) {
                    ...
                    if (safeGet(data)) {
                        data = $.toObj(data);
                        if (data.code === 200) {
                            console.log(`任务领取奖励成功`);
                        } else {
                            console.log(`任务领取奖励异常:${JSON.stringify(data)}\n`);
                        }
                    }
                }
            } catch (e) {
