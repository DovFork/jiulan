...
无毛了  凑个人头
by : https://github.com/shufflewzc/faker2/blob/main/jd_lzdz1_jx2.js
默认执行脚本。如果需要不执行
环境变量 NO_RUSH=false
...
[task_local]
#京选大牌 超会宠你
3 0 * * * jd_lzdz1_jx2.js, tag=京选大牌 超会宠你, img-url=https://raw.githubusercontent.com/58xinian/icon/master/jd_lzdz1_jx2.png, enabled=true
===================Loon==============
[Script]
cron "3 0 * * *" script-path=jd_lzdz1_jx2.js, tag=京选大牌 超会宠你
===============Surge===============
[Script]
京选大牌 超会宠你 = type=cron,cronexp="3 0 * * *",wake-system=1,timeout=3600,script-path=jd_lzdz1_jx2.js
====================================小火箭=============================
京选大牌 超会宠你 = type=cron,script-path=jd_lzdz1_jx2.js, cronexpr="3 0 * * *", timeout=3600, enable=true
 */
...
!(async () => {
    ...
    authorCodeList = await getAuthorCodeList('https://ghproxy.com/https://raw.githubusercontent.com/jiulan/helpRepository/main/json/lzdz1_jx.json')
    if($.getAuthorCodeListerr === false){
        authorCodeList = [
            '8E544CDD476CD86C80C209F59E570461',
        ]
    }
        ...
        if ($.secretPin) {
            ...
            if ($.openCardStatus) {
                for (let i = 0; i < ($.openCardStatus.cardList1.length); i++) {
                    $.log("模拟上报访问记录")
                    await task('crm/pageVisit/insertCrmPageVisit', `venderId=643605&elementId=${encodeURIComponent(`去开卡`)}&pageId=4c1162ed00874ba5ac0447c30cc45f8a&pin=${encodeURIComponent($.secretPin)}`, 1)
                    await $.wait(2000)
                }
                t1TaskList = []
            ...
            $.log("->->->> 任务")
            await task("dz/openCard/saveTask", `activityId=${$.activityId}&pin=${encodeURIComponent($.secretPin)}&actorUuid=${$.actorUuid}&type=2&taskValue=10032143277768`)
        }
    }
}
